...
4. **main.js → routeur + pages** ✅ (structure en place)  
   ...
   - **En cours.** Modules `features/` créés : `legal.js`, `landing.js`, `offers.js`, `auth.js`, `dashboard.js`, `checkout.js`, `builder.js` (pages importent depuis ces modules). `main.js` réduit (~5780 lignes) ; objectif < 400 en poursuivant l’extraction (checkout, builder, app). Fait : dashboard, checkout, builder. Constantes : `constants/builder.js`. Routeur : `navigateToBuilder`, `navigateToLanding`.

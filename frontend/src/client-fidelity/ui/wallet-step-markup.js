import { detectWalletPlatform, walletCtaPillClasses, walletDetectHintText } from "../../utils/walletPlatform.js";
...
export function renderWalletStepMarkup(esc, options = {}) {
  ...
  const appleClass = esc(walletCtaPillClasses(platform, "apple"));
  const googleClass = esc(walletCtaPillClasses(platform, "google"));
  const appleFirst = platform !== "android";

  const appleBlock = `
              <span class="fidelity-cta-wrap fidelity-cta-wrap--full">
                <a href="#" id="fidelity-v2-apple" class="${appleClass}" aria-label="Ajouter à Apple Wallet">
                  ${appleSvg()}
  ...
  const googleBlock = `
              <span class="fidelity-cta-wrap fidelity-cta-wrap--full">
                <a href="#" id="fidelity-v2-google" class="${googleClass}" aria-label="Ajouter à Google Wallet">
                  ${googleSvg()}
  ...
  const buttonsRow = appleFirst ? `${appleBlock}${googleBlock}` : `${googleBlock}${appleBlock}`;

  const confirmDesc =
    platform === "desktop"
      ? "Après avoir ajouté la carte depuis ton téléphone (Apple ou Google Wallet), confirme ici pour masquer ce bloc — la roue et les missions restent accessibles en dessous."
      : "Après avoir suivi Apple ou Google Wallet, tu peux confirmer ici pour masquer ce bloc — la roue et les missions restent accessibles en dessous.";
...
${buttonsRow}
            ...
            <div class="fidelity-v2-wallet-confirm" id="fidelity-v2-wallet-confirm-block">
              ...
              <p class="fidelity-v2-wallet-confirm-desc">${esc(confirmDesc)}</p>

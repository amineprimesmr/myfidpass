 ...
 * L'étape « Votre carte est prête » (step 5) affiche card beam + création de compte si onAccountCreated est fourni.
 */
...
<div class="builder-onboarding-logo-drop-zone" id="builder-onboarding-logo-drop-zone" role="button" tabindex="0" aria-label="Glisser-déposer une image, cliquer ou coller (Ctrl+V)">
  ...
  function renderAccountStepContent() {
    ...
    if (alreadyLoggedIn) {
      return `<div class="landing-onboarding-card-beam"><p class="landing-onboarding-card-beam-subtitle">Votre carte fidélité est en cours de préparation…</p><div class="landing-onboarding-card-beam-wrap"><iframe class="landing-onboarding-card-beam-frame" src="/card-beam-reversed.html" title="Animation création carte" referrerpolicy="strict-origin-when-cross-origin"></iframe></div><button type="button" class="builder-onboarding-btn" data-action="account-continue">Voir ma carte</button></div>`;
    }
    const socialBlock = (googleClientId || appleClientId) ? `<p class="landing-onboarding-account-divider">Ou continuer avec</p><div class="landing-onboarding-account-social">${googleClientId ? `<div id="builder-onboarding-google-btn" class="landing-onboarding-google-wrap"></div>` : ""}${appleClientId ? `<button type="button" id="builder-onboarding-apple-btn" class="landing-onboarding-btn-apple" aria-label="Continuer avec Apple"><span class="landing-onboarding-apple-icon" aria-hidden="true"></span>Apple</button>` : ""}</div><p class="landing-onboarding-account-divider">Ou avec votre e-mail</p>` : "";
    return `<div class="landing-onboarding-card-beam"><p class="landing-onboarding-card-beam-subtitle">Votre carte fidélité est en cours de préparation…</p><div class="landing-onboarding-card-beam-wrap"><iframe class="landing-onboarding-card-beam-frame" src="/card-beam-reversed.html" title="Animation création carte" referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="landing-onboarding-account-form"><p class="landing-onboarding-account-title">Créez votre compte pour accéder à votre carte</p><p id="builder-onboarding-account-error" class="landing-onboarding-account-error hidden" role="alert"></p>${socialBlock}<label for="builder-onboarding-email" class="landing-onboarding-account-label">Email</label><input type="email" id="builder-onboarding-email" class="landing-onboarding-account-input" placeholder="vous@exemple.fr" autocomplete="email" /><label for="builder-onboarding-password" class="landing-onboarding-account-label">Mot de passe (8 caractères min.)</label><input type="password" id="builder-onboarding-password" class="landing-onboarding-account-input" placeholder="••••••••" autocomplete="new-password" minlength="8" /><button type="button" id="builder-onboarding-register-btn" class="landing-onboarding-card-beam-cta">Créer mon compte</button></div></div>`;
  }
  ...
  function render() {
    ...
    const navLabel = getNavButtonLabel();
    const nextBtn = navLabel ? `<button type="button" class="builder-onboarding-btn" data-action="next">${navLabel}</button>` : "";
    const nav = nextBtn ? `<div class="builder-onboarding-nav">${nextBtn}</div>` : "";
  ...
  function bindAccountFormHandlers() {
    ...
    registerBtn?.addEventListener("click", async () => {
      ...
      if (!isValidEmailForAccount(email)) { showError("Adresse e-mail invalide (ex. vous@exemple.fr)."); emailInput?.focus(); return; }

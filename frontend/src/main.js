...
function triggerRouteViewEnter(el) {
  if (!el) return;
...
function initRouting() {
  ...
  if (route.type === "checkout") {
    ...
    if (checkoutAppEl) {
      ...
      triggerRouteViewEnter(checkoutAppEl);
    }
    if (builderHeader) {
  ...
  if (route.type === "templates") {
    ...
    if (landingTemplates) {
      ...
      triggerRouteViewEnter(landingTemplates);
      initBuilderPage();
    }
    updateAuthNavLinks();
    ...
    if (landingMain) {
      ...
      triggerRouteViewEnter(landingMain);
    }
    if (landingLegal) landingLegal.classList.add("hidden");
    const landingIntegration = document.getElementById("landing-integration");
    if (landingIntegration) landingIntegration.classList.add("hidden");
...
function initAuthPage(initialTab) {
  ...
  document.getElementById("auth-reset-form")?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (errEl) {
        errEl.textContent = data.error || "Lien invalide ou expiré. Demandez un nouveau lien.";
        errEl.classList.remove("hidden");
      }
    } catch (_) {
      if (errEl) {
  ...
  function showSocialHint(msg) {
    if (loginError) { loginError.classList.add("hidden"); loginError.textContent = ""; }
    if (authSocialHint) {
      authSocialHint.textContent = msg || "";
      authSocialHint.classList.toggle("hidden", !msg);
    }
  }
  if (!authGoogleClientId) {

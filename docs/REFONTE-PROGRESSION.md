...
4. **main.js → routeur + pages** ✅ (structure en place)  
   ...
   - **En cours.** Modules `features/` créés : `legal.js`, `landing.js`, `offers.js`, `auth.js`, `dashboard.js`, `checkout.js`, `builder.js`, `app.js` (pages importent depuis ces modules). `main.js` réduit (~870 lignes) ; objectif < 400 en poursuivant l’extraction (checkout, builder, app). Fait : dashboard, checkout, builder, app. Constantes : `constants/builder.js`. Utils : `utils/slugify.js`. Routeur : `navigateToBuilder`, `navigateToLanding`. À faire : main < 400 (extraire bloc App Carte / fidelity client + landing hero).

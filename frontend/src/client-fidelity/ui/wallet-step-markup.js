import { detectWalletPlatform, walletDetectHintText } from "../../utils/walletPlatform.js";
...
function confirmDesc(platform) {
  if (platform === "ios") {
    return "Après avoir ajouté la carte avec Apple Wallet, tu peux confirmer ici pour masquer ce bloc — la roue et les missions restent accessibles en dessous.";
  }
  if (platform === "android") {
    return "Après avoir ajouté la carte avec Google Wallet, tu peux confirmer ici pour masquer ce bloc — la roue et les missions restent accessibles en dessous.";
  }
  return "Après avoir ajouté la carte depuis ton téléphone (Apple ou Google Wallet), confirme ici pour masquer ce bloc — la roue et les missions restent accessibles en dessous.";
}

function confirmLater(platform) {
  if (platform === "desktop") {
    return "Pas encore ? Utilise les boutons ci-dessus, puis reviens cliquer ici.";
  }
  return "Pas encore ? Utilise le bouton ci-dessus, puis reviens cliquer ici.";
}

/**
 * Bloc « Ajoute ta carte au Wallet » (v2 client fidélité).
 * @param {(s: string) => string} esc
 * @param {{ platform?: 'ios' | 'android' | 'desktop' }} [options]
 */
export function renderWalletStepMarkup(esc, options = {}) {
  const platform = options.platform ?? detectWalletPlatform();
  const hint = esc(walletDetectHintText(platform));

  const appleOnly = `
              <span class="fidelity-cta-wrap fidelity-cta-wrap--full">
                <a href="#" id="fidelity-v2-apple" class="fidelity-cta-pill fidelity-cta-pill--wallet-single" aria-label="Ajouter à Apple Wallet">
                  ${appleSvg()}
  ...
  const googleOnly = `
              <span class="fidelity-cta-wrap fidelity-cta-wrap--full">
                <a href="#" id="fidelity-v2-google" class="fidelity-cta-pill fidelity-cta-pill--wallet-single" aria-label="Ajouter à Google Wallet">
                  ${googleSvg()}
  ...
  const appleDesktop = `
              <span class="fidelity-cta-wrap fidelity-cta-wrap--full">
                <a href="#" id="fidelity-v2-apple" class="fidelity-cta-pill fidelity-cta-pill--wallet-equal" aria-label="Ajouter à Apple Wallet">
                  ${appleSvg()}
                  <span class="fidelity-cta-pill-label">Apple Wallet</span>
                  <span class="fidelity-cta-pill-chevron" aria-hidden="true">›</span>
                </a>
  ...
  const googleDesktop = `
              <span class="fidelity-cta-wrap fidelity-cta-wrap--full">
                <a href="#" id="fidelity-v2-google" class="fidelity-cta-pill fidelity-cta-pill--wallet-equal" aria-label="Ajouter à Google Wallet">
                  ${googleSvg()}
                  <span class="fidelity-cta-pill-label">Google Wallet</span>
                  <span class="fidelity-cta-pill-chevron" aria-hidden="true">›</span>
                </a>
  ...
  return `
        <section class="fidelity-v2-card fidelity-v2-step" id="fidelity-v2-wallet">
          ...
          <div class="fidelity-v2-step-body">
            ...
            <p class="fidelity-wallet-detect-hint" role="note">${hint}</p>
...
${buttonsRow}
            ...
            <div class="fidelity-v2-wallet-confirm" id="fidelity-v2-wallet-confirm-block">
              ...
              <p class="fidelity-v2-wallet-confirm-desc">${esc(confirmDesc(platform))}</p>
              <button type="button" class="fidelity-v2-wallet-confirm-btn" id="fidelity-v2-wallet-confirm">Oui, j’ai ajouté ma carte</button>
              <p class="fidelity-v2-wallet-confirm-later">${esc(confirmLater(platform))}</p>

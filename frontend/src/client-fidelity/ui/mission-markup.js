...
/** Deux lignes de titre (style réf. « Content / Creation »). */
function missionTitleLines(label) {
...
function missionIconSvg(theme, uid) {
  const common =
    'class="fidelity-mission-card__svg" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"';
  switch (theme) {
    case "google":
...
function ctaArrowSvg() {
  return `<svg class="fidelity-mission-card__cta-arrow-svg" width="11" height="11" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M7 17L17 7M17 7H8M17 7v9" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
}
...
export function renderEngagementActionsMarkup(actions, esc) {
  return actions
    .map((a, i) => {
      ...
      const titleBlock = `
              <div class="fidelity-mission-card__title-stack">
                <span class="fidelity-mission-card__title-line1">${esc(line1)}</span>
                ${line2 ? `<span class="fidelity-mission-card__title-line2">${esc(line2)}</span>` : ""}
              </div>`;
      ...
      const inner = `
              ...
              <div class="fidelity-mission-card__banner-art">${artInner}</div>`;

/**
 * Scroll-Driven 3D Image Tube with React Three Fiber
 * Code from https://github.com/matdn/helmet - copy/paste identique
 */
...
function GridPlane({ targetCenterUv }) {
  ...
  useFrame((state) => {
    ...
    material.uniforms.uCenter.value.lerp(targetCenterUv.current, 0.08);
  });
...
function HelmetModel({ tubeAngleRef }) {
...
export function FiberScene() {
  ...
  const setTooltipFromClientPoint = useCallback((clientX, clientY) => {
  ...
  return (
    ...
    >
      ...
      <div className="helmet-whiteEdgeGradient" aria-hidden="true" />
      {hoveredProject && (
        <div className="helmet-projectTooltip" ref={tooltipElRef} role="status" aria-live="polite">
          {hoveredProject}
        </div>
      )}

      <div className="helmet-customCursor" ref={cursorElRef} aria-hidden="true" />
      <Loader />
    </div>
  );
}
